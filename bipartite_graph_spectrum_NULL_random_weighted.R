...
plot_distr_null <- function(df,nvalue,networkname="",title="",nestedvalue=""){
  ...
  if (nestedvalue!="")
    plot <- plot+geom_vline(data = data.frame("val"=nestedvalue), aes(xintercept = val), 
                            color = "red", size=0.5,alpha=0.5)+ggtitle(title)
...
WRG_build <- function(adjmatrix,nodes_guild_a,nodes_guild_b,num_links)
{
  ...
  N <- na*nb
  p <-  W/(N+W)   # Modelo Garlaschelli adaptado a bipartitas
...
store_model_results <- function(dp,num_links,num_nodes,magnitudes,weighted){
  ...
  if (weighted){
    mr$spect_rad_weighted <- dp$lpl_weighted_spect_nulls$values[1]
    mr$adj_weighted_energy <- AdjEnergy(dp$lpl_weighted_spect_nulls$values)
  ...
  if(typemodel == "WSYTR"){
    ...
    for (i in 1:nrow(wmatrix))
      for (j in 1:ncol(wmatrix)){
        if (wmatrix[i,j]>0)
          wmatrix[i,j] <- sum(rbinom(totweight,1,wmatrix[i,j]/totwmatrix))
      }
    incidmatrix <- wmatrix
  }
...
create_nested_model <- function(na,nb,nlinks){
  if (na < nb ){
    nr <- na
    nc <- nb
  } else {
  ...
  height <- round(2*rnl/(nr-1))
  for (i in (2:nr)){
    if (rnl==0)
      ntoks = 0
    else{
    ntoks <- max(0,min((height-i+2),rnl))
    rnl <- max(0,rnl-ntoks)
    if ((rnl>0) && (ntoks==0)){
      ntoks <- rnl
      rnl=0
...
#lnetw <- c("RA_HP_023.csv")
lnetw <- c("M_SD_002.csv")
seed <- 1000
num_experiments <- 100
ppi <- 300
plottofile <- TRUE
...
for (netw in (lnetw)){
  dfnested <- data.frame("network"=c(),"model"=c(),"NODF"=c(),"wine"=c())
  result_analysis <- analyze_network(directory = datadir, netw, guild_a = "Plant", guild_b = "Pollinator", only_NODF = FALSE)
  ...
  dfnested <- rbind(dfnested,data.frame("network"=netw,"model"="NETWORK",
                                        ...
                                        "wine"=result_analysis$nested_values["wine"]))
  ...
  nstspect_rad <- eigen(nstadj_sq_matrix)$values[1]
  ...
  mnames <- c("RND","MGEN","SHUFFLE","VAZ")
  if (weighted_network)
    mnames <- c(mnames,c("SWAP","WSYTR","SYTR","WRND","BSHUFFLE","BVAZ"))
  ...
  if (plottofile)
  {
    ...
    if (weighted_network)
      ...
      for (mname in c("BSHUFFLE","BVAZ"))
        modresults <- remove_model_data(modresults,mname)
    for (mname in c("WRND","WSYTR"))
      modresults <- remove_model_data(modresults,mname)

    if (weighted_network){
      pd_weighted_adj_energy <- plot_distributions(modresultsweighted,"adj_weighted_energy","Weighted Energy",adj_weighted_energy,netw,fract_dummy)
      pd_weighted_lpl_energy <- plot_distributions(modresultsweighted,"lpl_weighted_energy","Weighted Laplacian energy",lpl_weighted_energy,netw,fract_dummy)
      pd_weighted_spect_rad <- plot_distributions(modresultsweighted,"spect_rad_weighted","Weighted spectral radius",spect_rad_weighted,netw,fract_dummy)
    }
    #modresults[['WRND']] <- NULL  # WRND only makes sense for weighted laplacian energy
